...
export default function QuoteForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("userId", "123456"); // TODO: sostituire con id utente loggato
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="lg:col-span-2">
        <div className="bg-white/10 border border-white/5 backdrop-blur-xl rounded-2xl shadow-2xl p-6 lg:p-8">
          <div className="flex items-center justify-between gap-3 mb-6">
            ...
            <span className="px-3 py-1 text-xs rounded-full bg-emerald-500/10 text-emerald-200 border border-emerald-400/20">
              PDF in un click
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-semibold text-white">Linee di costo</h3>
                </div>
                <button
                  type="button"
                  onClick={addItem}
                  className="inline-flex items-center gap-2 rounded-xl border border-emerald-400/40 bg-emerald-500/10 px-3 py-2 text-sm font-medium text-emerald-100 hover:bg-emerald-500/20"
                >
                  + Aggiungi riga
                </button>
              </div>
              ...
              <div className="space-y-3">
                {items.map((item, i) => (
                  <div
                    key={i}
                    className="rounded-2xl border border-white/10 bg-slate-900/50 p-4 shadow-lg shadow-slate-900/40"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex flex-col items-end justify-between gap-2">
                        <p className="text-sm text-slate-300">Tot: {currency.format(item.quantity * item.unitPrice || 0)}</p>
                        {items.length > 1 && (
                          <button
                            type="button"
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center justify-between gap-3 rounded-xl border border-dashed border-white/15 bg-slate-900/60 px-4 py-3 text-sm text-slate-300 hover:border-emerald-400/40 cursor-pointer">
                <div>
                  <p className="font-medium text-white">Carica logo</p>
                  <p className="text-xs text-slate-400">PNG, JPG, massimo 2MB</p>
                </div>
                <span className="rounded-lg bg-white/10 px-3 py-1 text-xs text-white">Scegli file</span>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={isLoading}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-6 py-3 text-white font-semibold shadow-lg shadow-emerald-500/30 transition hover:bg-emerald-400 disabled:opacity-70 disabled:cursor-not-allowed"
              >
      ...
      <aside className="space-y-4">
        <div className="rounded-2xl border border-white/5 bg-white/5 backdrop-blur-xl p-5 shadow-2xl">
          <p className="text-xs uppercase tracking-[0.25em] text-slate-400 mb-2">Riepilogo</p>
          <h3 className="text-xl font-semibold text-white mb-4">Totali preventivo</h3>
